...
Если сохранённый план **не пуст**, агент добавляет в шаблон системного промпта заголовок **`### Current todo checklist`** и строки Markdown-списка. Встроенные шаблоны собираются из секций под **`internal/prompts/sections/`**, а файлы из **`prompts.dir`** сохраняют прежнюю форму с настраиваемыми именами **`agent.md`**, **`plan.md`**, **`docs.md`** и **`ask.md`**. Вставка выполняется через `{{if .TodoList}}` … `{{end}}` и пропускается, когда список пуст. Перед **каждым** вызовом LLM в рамках одного запроса **`session/prompt`** FoxxyCode обновляет системное сообщение, поэтому созданный или изменённый ранее в том же эпизоде ReAct список сразу остаётся видимым.
