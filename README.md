...
FoxxyCode — совместимая с distroless **среда выполнения агента**: её можно помещать в минимальные образы (`scratch`, `distroless`, рабочие каталоги только для чтения), не устанавливая полноценную системную оболочку. Инструменты **`grep`** и **`glob`** используют системный ripgrep, когда он доступен, и переключаются на встроенные реализации на Go, когда его нет. Уровень среды (ACP RPC, сессии, промпты, провайдеры) не меняется, если ограничить набор инструментов или управлять агентом из автоматизации вместо IDE. Архитектура также рассчитана на **контейнерные кластеры** — множество экземпляров FoxxyCode в Docker с заданными оркестратором ограничениями, корневой ФС только для чтения и подключённым рабочим каталогом. При этом сохраняется **полный контроль над каждым контейнером**, как в системах класса agent OS или swarm-агентов, а не в едином общем пуле чатов.
...
В переменной **`TAGS`** для **`Makefile`** используйте **пробелы** (**`make build TAGS="http ui scheduler memory miniapps cli browser"`**), а в **`go build`** — **запятые** (**`-tags=http,ui,scheduler,memory,miniapps,cli`**).
...
Официальные CLI-сборки публикуются в **[GitHub Releases](https://github.com/hijera/foxxycode-agent/releases)** (например, **`foxxycode_0.9.3_linux_amd64.tar.gz`**). Каждый релиз содержит полный набор функций сборки **`make build TAGS="http ui scheduler memory miniapps cli browser"`**.
