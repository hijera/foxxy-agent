...
- The **live line does not repeat the row above it.** A reasoning row is present exactly when the live status is **`thinking`** - that is what derives it - and it already says the word and ticks its own duration, so the typing line under it keeps the three dots and drops the text. Every other status keeps its phrase.
