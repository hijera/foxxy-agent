...
- **`GET /v1/models`** merges FoxxyCode profiles and YAML backends in one list. Split by **`owned_by`**: **`foxxycode`** means session profiles **`agent`**, **`plan`**, and **`docs`** only. Any other **`owned_by`** marks a configured **`models[].model`** row (YAML backend).
- **`Mode`** lists **`agent`**, **`plan`**, and **`docs`**. Default is **`agent`**. **`plan`** uses the orange outline treatment; **`docs`** uses a blue outline treatment.
...
- For ReAct (**`agent`** / **`plan`** / **`docs`**), the UI sends **`metadata.model`** with the selected YAML **`id`**; the context-meter **`max_context_tokens`** for the ring follows that YAML row.
