...
- **Layout:** **`.miniapps-workspace`** is a full-height overlay with a catalog column and editor. The structured workflow view follows the ValeDesk mini-workflow reference: numbered typed steps, private sanitized source-session result, name/description/goal/author, inferred inputs, acceptance criteria, then Save draft, Test run, and Release actions. Separate **JSON** and generated **Run** tabs expose the portable program and operator form.
- **Lifecycle:** **Distill this session** polls the asynchronous job until an editable draft opens. The acceptance section accepts plain-language author expectations; **Generate expected result with LLM** uses the draft's fixed model binding, or the configured default model when the draft has none, to save `expected_result`, `acceptance_criterion`, and an executable prompt check into the JSON. Draft changes remain unversioned; a passing test for the exact revision plus sanitization is required before release. Released catalog rows run their exact immutable version.
...
- **Responsive:** at **`max-width: 1199px`** catalog and editor stack; at phone widths the catalog becomes a short horizontal strip and authoring columns collapse without horizontal page overflow.
