...
- **Language:** the same tab, directly under the swatch grid — a native **`<select>`** (**`AppearanceLanguagePicker`**, **`data-testid="appearance-language-select"`**) offering **Auto** plus every locale in **`ui/i18n/locales.ts`**. Unlike the theme, it **does** persist to the config (**`ui.locale`**), because the editor plugins read that key; **Auto** stores **`""`** and clears the **`foxxycode_ui_lang`** cookie. Full contract in [`docs/surfaces/web-ui.md`](docs/surfaces/web-ui.md).
...
The **Settings** drawer (**`#/settings`**, **`Settings.tsx`**) is **tabbed**, not one long sheet. Tabs are derived from the live config JSON Schema (**`deriveSettingsSections`** over **`/foxxycode/config/schema`**): each top-level property is a tab (label from its **`title`**) in the schema's own order, which puts **Context compaction** right after **ReAct agent** (**`rootOrder`** in **`internal/config/ui_schema.go`**), the rarely edited tail (**`scheduler`**, **`prompts`**, **`instructions`**, **`logger`**, **`sessions`**, **`gateways`**) folds into a single **System** tab, and three client-side tabs are placed **first** - **General** (the UI language picker, the default tab), **Appearance** (the theme picker) and **Sessions** (**`sessions_manager`**, the session management table below). All three render before the schema has loaded. There is no separate Appearance/Skills flyout or **`settings-dock-cluster`** side panel — both are tabs now.

- **Saving**: there is **no per-section save button**. The drawer footer keeps only **Reload** (**`data-testid="settings-reload"`**) and **Save all** (**`data-testid="settings-save"`**, PUTs the whole config after `/foxxycode/config/validate`). The **Appearance** theme picker is client-side only and never touches the config PUT; the **language** select in the same tab writes **`ui.locale`** immediately and mirrors the pick into the loaded doc, so **Save all** cannot put a stale value back.
...
- **Sessions** tab (**`SessionsManager.tsx`**, section kind **`sessions`**, id **`sessions_manager`** because **`sessions`** is already the config key for the storage directory and stays in **System**) is the stored history as a **table**, API-driven over **`/foxxycode/sessions`** and outside **Save all**. Reference capture: **`docs/assets/sessions-management-table-dark-1280.png`**.
  ...
  - **Scope**: the SPA sends only **`{ids}`** - exactly the ticked rows - plus the **`cwd`** of the project scope below when it is on. The endpoint also accepts **`{scope:"all", except}`** for API clients that want the whole stored history without paging; that shape refuses an **`except`** naming no stored session and spares the **ancestors** of what it keeps (see **`docs/reference/http-api.md`**), but no button in the table reaches for it.
  - **Project scope** (fork): an editor plugin runs one server per project over a session home every project shares, so the table follows the History drawer's **`sessions-project-only`** toggle. **`App.tsx`** passes **`sessionsScope`** (the same **`hostProjectRoot`** / **`sessionsProjectOnly`** state and **`changeSessionsProjectOnly`** setter the drawer uses) through **`Settings`** and **`SettingsSection`**; with the toggle on, the list request carries **`cwd`** and the bulk-delete body carries the same **`cwd`**, which the server enforces rather than trusting the rows it was sent. The toolbar repeats the toggle (**`sessions-manager-project-only`**) between the search field and the action, reusing the drawer's **`.sessions-project-scope`** markup with **`.sessions-manager-scope`** removing its row padding and divider. Outside an editor plugin the preference starts **off** (the drawer's default), so the table lists every workspace until the operator switches it on; a server that reports no workspace path gives no host root, hence no toggle and no **`cwd`**.
...
- **Server activity and local reader lifetime are separate.** On session open and reconnect, the SPA reads **`GET /foxxycode/sessions/{id}/activity`** for the selected id, independently of the filtered or paginated History list. **`chat/useSessionTurnActivity`** skips both activity and queue hydration while this tab's own POST awaits admission. An active turn keeps Stop and queue controls available without a local reader. A reader closing does not mark the server idle; a **`turn_ended`** overlapping this tab's pending or admitted POST requires fresh REST activity confirmation before the SPA declares idle.
- The shadow transcripts form a **small LRU** (**`ShadowTranscriptCache`** in **`external/ui/src/ui/chat/sessionTranscriptCache.ts`**, cap **`SHADOW_TRANSCRIPT_CACHE_CAP`** = 3 non-pinned entries). The **viewed** session and every session with a **live composer stream** (own **`POST`** or a relay attach) are **pinned**; the least recently used entries beyond the cap are dropped whenever a session is picked, the user goes home, a stream finishes, or **`loadMessages`** stores a fresh snapshot. An evicted session is simply **re-fetched** from **`GET /foxxycode/sessions/{id}/messages`** on the next visit, the path a finished dialog already takes today, so eviction costs no extra request. A **`loadMessages`** response that lands after the viewer moved to another session only refreshes that session's shadow; it never paints under the current route.
...
- **Stop** (**`#btn-send`** as stop) calls **`POST /foxxycode/sessions/{id}/cancel`** and aborts this tab's own streaming **`fetch`** of the turn (the POST or the relay) as soon as the request is sent, not after the answer: over HTTP/1.1 a browser keeps six connections to a host for all of its tabs, a few open tabs can hold every one of them with event and turn streams, and the cancel request then waits for the connection this reader holds. The server turn does not depend on the reader. A failed request leaves a visible error and Stop available for a retry, and the tab rejoins the still running turn through the relay. Success acknowledges cooperative cancellation, not turn completion; server activity confirms idle. The server **persists** assistant tokens already received for that turn when cancel lands mid-stream (**`internal/llm`** stream implementations return a partial **`Response`** with **`context.Canceled`** wrapped, then **`internal/agent`** **`Run`** appends **`RoleAssistant`** before surfacing **`StopReasonCancelled`**).
...
- **`404`** from **`GET /foxxycode/scheduler/jobs`** means the server build has no scheduler HTTP surface; **`503`** means **`scheduler.enable`** is false for that process. The drawer shows a plain-language line instead of crashing.
...
Repo links appear as plain text links (**`API docs`**, **`Website`**) on the left of the **Settings footer**, before the Reload and Save buttons (**`.settings-footer-link`**; the last one takes **`margin-right: auto`**). **API docs** (**`/docs/`**) and **Website** (**`https://hijera.github.io/foxxy-agent/`**) open in a **new tab** (`target="_blank" rel="noopener"`). Inside the VS Code embed the webview blocks new windows, so **`embedExternalLinks.ts`** claims such clicks and posts **`{ type: "foxxycode:openExternal", url }`** to the host; the IntelliJ panel cancels the JCEF popup and opens the system browser.
...
**A shell command is input, not body text.** **`run_command`** / **`ssh_run_command`** use the **`shell`** preview kind: the command sits in an inset rounded block inside the card behind an accent **`$`** prompt, and the control that copies it sits on that same line, in the block's last column. The card header above it never carries a second copy control. For a local call that header is the interpreter the server actually runs (**`/usr/bin/bash`**, **`cmd.exe`**), published as **`shell`** on **`GET /foxxycode/workspace/context`** and held in the process-wide **`chat/hostShell.ts`** store the way the locale is held; a server that reports none, and a remote **`ssh_run_command`** whose interpreter lives on somebody else's host, keep the generic label.

**Output carries no label, and a failure is said once.** The returned body attaches directly under the call, with no **Result** header and no divider - the preview viewport drops its closing edge when a result follows, so the two panels are one surface: it is the only thing there, and naming it was chrome. One surface means **one** gap at the seam: a command block already carries its own **10px** inset inside the card, so the output under it drops its top padding (**`.foxxycode-tool-call-body--connected-result > .permission-preview:has(.permission-preview-shell) + .tool-call-result-card .tool-call-result-content`**); previews whose body runs edge to edge, a diff or a code panel, keep theirs. A call that **failed** says so on its summary row instead - **`.tool-failed-marker`** renders **(failed)** / **(ошибка)** in the theme's deletion red, between the target and the duration - so the failure is readable while the row is still collapsed.

**Browser and SVN rows keep their own chrome inside the same group.** Their icon opens **`.thinking-head`**, so an icon and a long URL or path never break the row onto two lines, and their label is the operation (**Open page**, **SVN · Update working copy**) rather than a **`tool.name.*`** entry. An SVN call reports a failure with its **`.svn-summary-error`** pill instead of the marker - it detects an **`error:`** result that arrived as completed, not only the failed status - and a browser call whose result starts with **`error:`** gets the **`.tool-failed-marker`** as well. A **`spawn_agent`** row whose background chip already names the agent shows no target beside the label.
...
**A web search is read as its hits, all of them.** **`websearch`** renders its answer as a Markdown list of links (**`webSearchResultMarkdown`** in **`chat/webToolResults.ts`**), each snippet on its own line under its link (a hard break, not a run-on after it), closed by the tool's **`hint`**. The row's preview is the first 19 lines of the output and the engine report that leads the answer fills them - the cut usually lands on **`"results": [`** with not one hit whole, which is how the row came to print raw JSON behind a **More…** control. So **opening the row is the request for the results**: the first **`toggle`** to open fetches the saved answer once (**`GET /foxxycode/sessions/{id}/tool-calls/{toolCallId}`**), a muted **Loading…** line (**`.tool-result-loading`**) stands in meanwhile, and the whole list renders at its natural height with **no More / Less** and no capped viewport. A closed row costs no request; only a failed fetch brings back the ordinary **More…** to try again. The **header carries every parameter of the search** - the query, **`page N`** and **`max N`** with the tool's defaults filled in (page 1, 15 rows, capped at 25) and **`site …`** when one was named - and the first line of the body (**`.web-search-engines`**, **`webSearchReport`**) says how each engine answered: **`bing: 10`** for one that answered, **`brave: blocked (http 429)`** in the failure colour for one that did not. The report is read out of the preview as well as the full answer, since it is whole even where no hit is.

**A scheduler call is read as what happened to which job.** The **`foxxycode_scheduler_*`** tools take a **`job_id`** and answer with a line of JSON, and the row printed both as they were - **`{"job_id": "ai-news-digest"}`** over **`{"job_id":"ai-news-digest","paused":false}`**. They render **`SchedulerToolCard`** (**`messages/SchedulerToolCard.tsx`**, model in **`chat/schedulerToolDisplay.ts`**) instead of the argument preview and the result panel: the preview bar with the plan rows' status mark, the job id (**`old → new`** for a rename) and, as its meta, **what happened** - **`resumed`** / **`снято с паузы`**, **`run started`** / **`запуск принят`**, **`was not running`** for a Stop of an idle job, **`created`**, **`updated`**, **`deleted`**, **`paused`**, **`replaced`**. A create, replace or patch lists under the bar the fields it set; **`job_get`** lists the job - description, the cron with its reading (**`0 8 * * * · At 08:00, every day`**), state, next run (UTC), mode, model, folder - and its instruction as Markdown in an inset panel; **`jobs_list`** is one row per job (id, description, schedule, state) with the count as meta; **`job_runs`** one row per run (local start, status, duration). The reading tools fetch the whole answer when the row opens, as a search does. The row names the job beside the label (**`toolCallTargetText`** reads **`job_id`** for every scheduler tool). A failed call keeps its error as raw text. **Every scheduler tool has a **`tool.name.*`** entry in every dictionary** - **`foxxycode_scheduler_job_resume`** shipped without one and the row read the raw id; **`TestEverySchedulerToolIsNamedInTheWebUI`** (**`external/scheduler/tools/names_ui_test.go`**) now holds the registry to the dictionaries.
...
**Browser tools** (`foxxycode_browser_*`) keep the compact disclosure row and timer, with a browser icon and localized action label. Navigation shows its URL and places the captured page screenshot in **Result**; screenshots enlarge on click. Click, hover, fill, and scroll-to-element show a highlighted CSS selector; fill also shows the input text. Evaluate shows formatted, syntax-highlighted JavaScript above its JSON result. Code and long text use measured **More / Less** controls (180px clipped, 360px scrolling); copying copies displayed text. Read-page preserves the outline and indentation, page-log distinguishes warnings and errors, and inspect renders storage/timing/memory reports as tables. Permission previews reuse the same argument presentation.

For scroll offsets, draw a 260px-wide browser window with a visible rectangular border and a decorative top bar containing window dots and an address field. The viewport below contains an arrow from its center in the X/Y direction and signed pixel offsets inside its lower edge. Positive X points right; positive Y points down; both axes produce a diagonal, with the ratio preserved. Zero offsets have no arrow and say **No displacement**. A nonempty selector takes precedence and hides the diagram. The tool applies X/Y relative to the current scroll position. Screenshot capture can be disabled or unavailable without creating a broken image. Full results loaded from history must use the same renderer; errors retain every diagnostic line.

### SVN tool cards

**SVN tools** (`svn_info`, `svn_status`, `svn_diff`, `svn_log`, `svn_list`, `svn_add`, `svn_revert`, `svn_resolve`, `svn_update`, `svn_commit`, `svn_switch`, `svn_merge`, `svn_checkout`) use a branch icon and localized action heading in the existing disclosure row. The expanded card shows operation state, labeled arguments, paths and commit message, followed by a bounded, copyable result. An omitted scope is explicitly labeled **Entire working copy** where applicable. `svn_info` uses label/value rows; `svn_status` preserves all seven status columns and highlights text, property and tree conflicts; `svn_diff` preserves every line, including binary and property diagnostics, with addition/deletion backgrounds. Other output remains verbatim. Unknown status lines and malformed arguments remain readable rather than disappearing.

Transport completion does not imply SVN success: an `error:` result gets a failed state and an error badge visible while collapsed. Cancelled calls preserve partial output without a success badge. Pending approvals reuse the argument card with **Awaiting permission** and retain the existing permission controls. Truncated results are marked as partial, and loading full history uses the same renderer. All new labels have EN/RU translations; colors use existing theme tokens. At 600px and below, field labels stack above values and status captions wrap below paths. Visual checks: [before](docs/assets/svn-tools/before/svn-tools-dark-1280.png), [after](docs/assets/svn-tools/after/svn-tools-dark-1280.png), [light mobile](docs/assets/svn-tools/after/svn-tools-light-390.png).
...
- Buttons keep the backend option order: **Allow**, **Allow always**, optional **Always allow `<program>`**, **Reject**. The fork translates the labels client-side by **`optionId`** (**`chat/permissionOptionLabel.ts`**); the backend text is English and is only a fallback. The desktop toast's buttons go through the same function, so the toast and the card beside it never show different languages.
...
A context row (**`.composer-context-row`**) renders as the **first child** of **`.composer-card`**, above attachments and the field. It holds the environment chip, the workspace chips (**`.composer-context-chips`** is **`display: contents`**, so every chip is a direct flex item and only the overflow wraps), and the 24×24px improve-prompt button pinned to its right edge (top-right below 520px) — mirroring Claude Desktop's workspace chips. Implemented by **`WorkspaceChips.tsx`** (helpers in **`chat/workspaceContext.ts`**); data from **`GET /foxxycode/workspace/context`** (session header, or **`?path=`** preview before a session exists).
...
- **Open folder…** opens the project-styled **folder browser modal** (**`WorkspaceFolderModal.tsx`**, **`workspace-modal`**, centered over a dim backdrop): editable path field, **`..`** row, subfolder rows from **`GET /foxxycode/workspace/folders`** (row click **navigates into** the folder), footer **Cancel** / **Open** (picks the currently browsed folder). The browser starts at the **parent** of the current workspace.
...
- Active session: arc fills from stats; live **`usage_update`** SSE replaces both the current total (`used`) and the session's context window (`size`) immediately and refreshes detailed stats, so manual and automatic compaction reduce the displayed window without a reload. Both compaction engines (**`compaction.engine`** **`coddy`** / **`opencode`**) publish it. The live size overrides the initial model listing, including when a provider listing arrives late, survives stats refreshes, and is scoped to the session, selected model and configuration version. The tooltip may include usage lines but **never** a **`Model …`** line that duplicates **Mode** (the mode dropdown).
...
- **The button** is a **34x34px** circle (**`border-radius: 50%`**) on the shared glass tokens (**`--foxxycode-glass-panel-bg`** / **`-border`** / **`-backdrop`**), so all seven themes carry it without a per-theme rule. Its glyph is a **16px** stroked down arrow in **`currentColor`**. Its shadow is its **own, small** one (**`0 3px 10px`** plus **`0 1px 2px`**, lighter under **`[data-theme="light"]`**) and **not** **`--foxxycode-glass-panel-shadow`**, whose **`0 14px 44px`** cast is drawn for a composer-sized card and reads as a smudge under a circle this size.
...
- Idle **disabled** when message field empty; an active session turn shows **Stop** with an empty draft and **Queue** with text, whether or not this tab has a stream reader (see **`docs/surfaces/web-ui.md`**, section **Composer primary action**).
...
- **`Mode`** lists **`agent`**, **`plan`**, **`docs`**, **`ask`**, and **`debug`**. Default is **`agent`**. **`plan`** uses the orange outline treatment; **`docs`** uses a blue outline treatment; **`ask`** uses a green outline treatment; **`debug`** uses a red outline treatment.
- Selected mode is sent as top-level **`model`** in **`POST /v1/responses`**.
- **Mode is session state, synchronized both ways.** Changing **Mode** **`PATCH`**es **`mode`** on the active session immediately, so a profile picked without sending a turn is not lost; **opening a session** restores it from **`GET /foxxycode/sessions/{id}/messages`** field **`mode`**. A **new chat** is always **`agent`** — **New chat** and picking a client draft both reset it, and there is no mode cookie (unlike **Model**).
- **A switch the backend makes itself** — a plan run (**`metadata.runPlanSlug`**), or the model calling **`plan_exit`** — arrives as the named SSE event **`mode`** (**`currentModeId`**) on that turn's stream and on the composer relay, and repaints the pill. The post-turn transcript read adopts the session's mode as a fallback for a stream that was cut, but never over a mode the user switched **while** that turn was running (**`shouldAdoptServerModeAfterTurn`**). Without this the pill kept saying **`plan`** after **Implement the plan**, and the next turn's top-level **`model`** wrote **`plan`** back onto a session the backend had already moved to **`agent`**.
- The stored selection (**Mode**, **Model**, **Reasoning**) is applied **once per session open**, not on every transcript reconcile: **`loadMessages`** also runs after every turn, after a permission answer and on relay reconnect, and re-applying its snapshot reverted a pick the user had just made. Guard: **`shouldApplySessionSelection`** (**`chat/sessionSelectionApply.ts`**).
...
- For ReAct (**`agent`** / **`plan`** / **`docs`** / **`ask`** / **`debug`**), the UI sends **`metadata.model`** with the selected YAML **`id`**; the context-meter **`max_context_tokens`** for the ring follows that YAML row.
...
- **Stays in step with the configuration** — the list behind this menu is read once, at boot, so a model added while the page is open would otherwise be invisible until a reload. **`GET /foxxycode/events`** carries **`event: config_reloaded`** after every swap of the live configuration - a settings save, the agent's own **`config_commit`**, a skill install, an edit on disk that **`foxxycode serve`** picked up - and **`App.tsx`** bumps **`configEpoch`** on it, re-reading **`/v1/models`** and the slash commands in place.
...
- **Rows** (**`.at-range-lines`** > **`.at-range-line`**) are one logical line each in the monospace stack (**`--mono`**, 12px, line-height 1.5): a right-aligned gutter **`.at-range-line-no`** (min 3ch, 50% opacity, tabular, not selectable) and the code in **`.at-range-line-code`** with **`white-space: pre`**. Long lines never wrap; **`.at-range-lines`** scrolls horizontally inside the panel so the page keeps **`scrollWidth === clientWidth`**. Selected rows use **`.at-range-line--sel`** (accent at 18% over **`--foxxycode-blend-base`**); a start typed without an end highlights that one line.
...
- The panel counts as **open** only once the file loaded (**`GET /foxxycode/workspace/file`**, one fetch per settled path), so a colon in prose never flashes an empty panel; a path that does not resolve leaves it closed. **`Escape`** dismisses it for that mention, **`Enter`** is left alone and still sends. The **`@`** file menu gains one footer line **`.at-range-menu-hint`** (top border at 12% text tint) telling the user that **`:`** after a file picks lines.
