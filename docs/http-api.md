...
| POST | **`/coddy/enhance-prompt`** | JSON **`{"text"}`** — rewrites the user's draft prompt into a clearer, more specific one via a single direct LLM completion (no tools). The draft is treated only as source text to improve, never as a request to answer. JSON **`{"object":"coddy.enhance_prompt","text"}`**; **`400`** on blank text, **`502`**/**`503`** on LLM error/unavailability. The composer wand button (**`data-testid="composer-enhance-btn"`**) calls this and replaces the draft; **Ctrl+Z**/**⌘Z** restores the pre-enhance text. |
