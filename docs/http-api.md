...
| GET | **`/coddy/config/schema`** | JSON Schema (draft 2020-12) for the settings editor. Describes the same logical fields as **`ConfigJSON`** with **`description`**, **`default`**, and **`x-coddy-property-order`** for section order. **`httpserver`** is omitted (listen bind stays on the **`coddy http`** CLI). **`GET`** / **`PUT`** **`/coddy/config`** still return the full document including **`httpserver`** when present in YAML. |
