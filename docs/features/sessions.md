...
| Path | What it holds |
|---|---|
| `session.json` | id, `cwd`, `mode`, the session's model and reasoning overrides, the permission-mode override, the title (derived, or pinned by the user), `updatedAt`, the context handed over by `SessionStart` hooks, the run metadata of a scheduler or subagent bundle, and the activity counters behind the unread dot |
...
The rollback works from a snapshot each turn takes of the working directory before it starts, and that snapshot is bounded so a large checkout cannot run the server out of memory. It knows every file by size, modification time and mode, but holds content only for as many files as fit 32 MB, smallest first and none over 4 MB, and it tracks at most 200,000 files. After the turn only what changed is read. A file the turn changed whose earlier content the snapshot did not hold cannot be put back: `diffs/turn_<n>.json` marks it `before_unavailable`, the rollback leaves it as the turn left it rather than deleting it, and the note the branch reports names it. Earlier releases counted such a file as created by the turn, and the rollback removed it.
