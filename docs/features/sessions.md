...
| ACP editors | `session/load` restores `session.json` and `messages.json`, replays the turns and the tool call summaries, sends a `plan` update when `todos/active.md` exists, then the command catalog; `session/list` lists the bundles with an optional `cwd` filter, and `loadSession` is advertised whenever a store is configured. `foxxycode acp --session-id <id>` makes the next `session/new` reopen that bundle, or create one under that name ([ACP protocol](../reference/acp-protocol.md#sessionload)). |
