...
| `FOXXYCODE_TELEGRAM_API_BASE` | `internal/dryrun/network.go` | The Bot API origin the `--dry-run` Telegram probe talks to, for stand-ins and self-hosted gateways. | [Configuration](../getting-started/configuration.md#dry-run-probing-what-the-file-points-at) |
