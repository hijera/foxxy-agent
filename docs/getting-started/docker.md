...
```bash
export FOXXYCODE_IMAGE=ghcr.io/hijera/foxxy-agent:0.2.0
docker compose pull
docker compose up -d
```
...
```bash
export FOXXYCODE_IMAGE=ghcr.io/hijera/foxxy-agent:NEW_TAG
docker compose pull
docker compose up -d
```
...
3. **`scratch`** - only the binary and CA bundle; **`ENTRYPOINT`** **`/bin/foxxycode`**, default **`CMD`** **`serve -H 0.0.0.0 -P 12345`**.
