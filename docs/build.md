...
The [**Dockerfile**](../Dockerfile) uses the same idea: comma-separated tags via **`BUILD_TAGS`** (default **`http,scheduler,ui,memory,miniapps,gateway`**) and strips debug symbols with **`-ldflags "-s -w ..."`** in addition to the version **`X`** flag.
...
That compiles whatever the module default is **without** your local **`TAGS`**. For a known set of features (HTTP, UI, scheduler, memory, mini apps), clone the repo and use **`make build TAGS="http ui scheduler memory miniapps"`** (or **`go build -tags=...`** as above).
