...
Templates are **`internal/prompts/agent.md`**, **`plan.md`**, **`docs.md`**, **`ask.md`**, and **`debug.md`** (embedded by default or overridden via **`prompts.dir`**). Ask also includes built-in **`ask.openai.md`** and **`ask.gpt-oss.md`** variants. They use Go **`text/template`**.
