...
| `llm_stall_timeout_ms` | int | no | `60000` | How long a streamed LLM call that has **already produced output** may go without any sign of progress before the turn cuts it, keeping the partial answer and asking the model to continue. Covers what `llm_first_token_timeout_ms` cannot: that guard stops at the first token and never re-arms. An explicit `0` disables it; blocking (`stream: false`) transports are never guarded. Continuations count against `max_turns`. |
