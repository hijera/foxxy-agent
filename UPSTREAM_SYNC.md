...
| Ключи-переключатели | Upstream переименовал `cors.enabled` → `enable` и завёл новые как `enable`. Здесь везде `enabled`: шестнадцать ключей форка уже так читаются, и переименование молча сломало бы существующие конфиги. Конфиг от upstream при этом тоже работает: `enable` читается как `enabled` во всех секциях с таким переключателем (`internal/config/enable_alias.go`), `-t` даёт предупреждение, запись переписывает ключ в `enabled` |
...
2. `git log --oneline --no-merges 45aee1ec..upstream/main` — список кандидатов.
