...
## Волна `257a332b → 45aee1ec` (теги `1.1.2`–`1.1.25`) — портирована, PR на ревью
...
| PR 3 | `claude/upstream-wave-sessions-and-queue` | #233 · #236 #237 #238 · #239 · #240 | единый вид id и вложенные сессии субагентов, worktree в репозитории, кэш сохранения; строки инструментов; очередь сообщений; этот файл |
...
- **Компакция движком форка теряла сжатие на диске (PR 3).** `State.ReplaceMessagesAndPersist`,
...
- **Подсветка кнопки очереди не рисовалась и у upstream (PR 3).** `.composer-run-icon--queue`
  стоит в `styles.css` раньше `.composer-send-play` с той же специфичностью и проигрывала ей.
- **Ответ на позднее уточнение сливался с предыдущим (PR 3).** Прогон, которым граница хода
...
- **Переподключение к ходу рисовало его дважды (PR 3, только форк).** Форк при повторном
...
- Не проверено на этой волне: прогон на NeuralDeep (ключа в окружении не было), панели IntelliJ
  (JCEF, Chromium 104) и VS Code вживую — бандл проходит `check:compat`, SPA проверена в Chrome;
  `-race` (см. выше).
...
| Поле | Значение |
| --- | --- |
| **Дата** | 2026-09-14 (порт; PR на ревью) |
| **Синхронизировано до `upstream/main`** | `45aee1ec` (2026-09-14) |
| **Ближайший upstream-тег** | `1.1.25` |
| **Наш коммит-порт** | #91, #95 и PR 3 (`claude/upstream-wave-sessions-and-queue`), стеком; см. таблицу волны выше |
| **Гейты** | на каждом PR: `make test` (экспресс-прогон: `ui-build`, `tsc` и vitest — к PR 3 это 228 файлов и 1696 тестов, затем `go test` со всеми тегами), `make lint`, `make docs-check`. Матрица тегов, Windows и macOS — в CI |
| **Живой прогон** | #91: конфиг из Блокнота, UTF-16 и строка ошибки на бинарях `main` и ветки. #95: инструкции оператора в промпте, строгий стрим, вход на `serve` и прямой локальный клиент `http`, таблица сессий с `cwd` — на локальной заглушке OpenAI. PR 3: строки инструментов на фикстурной сессии до и после (тёмная, светлая, русский, 390 px) и очередь сообщений на живом ходе против заглушки, которая держит первый ответ открытым, — headless Chrome; кадр консоли снят из её экранного буфера. На NeuralDeep волна не гонялась: ключа в окружении этих сессий не было |
