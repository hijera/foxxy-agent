...
| этот | `claude/upstream-wave-serve-rename` | свод `serve`-переименования, gutter роя, этот файл | — |
...
- **`mcp.go` и `skills.go` называли флаги не тем командам.** `--skills-auto-discovery`
  ...
  четырёх, включая `http`. Комментарии приведены к регистрации флагов. (этот PR)
- **Инструкция зеркалирования `.cursor/rules`** велела копировать тело `.md` в `.mdc`
  ...
  стирает абзацы, которых нет во второй копии. Инструкция в `workflow.md` исправлена (этот
  PR); **само расхождение файлов старше этой волны и не сведено** — в `core-modules` у `.md`
...
| **Наш коммит-порт** | #82, #84, #85, #87, #88 и этот PR; #86 — вне upstream. См. таблицу волны выше |
