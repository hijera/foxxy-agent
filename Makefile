...
# `cd` rather than `npm --prefix`: under make's shell on Windows npm ignores the
# prefix when it looks for package.json and dies on the repo root, which broke
# `make ui-build` (and everything depending on it) on a Windows checkout. The
# editor-plugin targets below already use this form.
ui-build:
