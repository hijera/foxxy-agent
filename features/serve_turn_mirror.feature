Feature: a turn started outside the web UI is watchable in it
  ...
  Scenario: the assistant text reaches both surfaces
    Given a live session "sess_mirror"
    ...
    And a web client watching session "sess_mirror" receives "hello from telegram"
  ...
  Scenario: the mirror is released when the turn ends
    ...
    Then no relay is left registered for session "sess_mirror"
