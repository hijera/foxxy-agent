...
/**
 * Reads the foxxy `GET /coddy/ide/events` Server-Sent-Events stream on a background daemon
 * thread and delivers parsed [FoxxyEditEvent]s to [onEvent]. Reconnects with backoff until
 * [stop] is called. Pure `HttpURLConnection` (no extra deps), matching FoxxyProcessManager.
 */
class FoxxyIdeEventClient(
    private val baseUrl: String,
    private val onEvent: (FoxxyEditEvent) -> Unit,
) {
    private val log = logger<FoxxyIdeEventClient>()
    ...
    fun start() {
        if (running) return
        running = true
        thread = Thread({ loop() }, "foxxy-ide-events").apply {
            isDaemon = true
            start()
        }
    }
    ...
    private fun parse(payload: String): FoxxyEditEvent? {
        return try {
