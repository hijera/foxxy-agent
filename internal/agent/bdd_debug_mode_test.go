...
func (s *debugFeatureState) modelReceivesFullTools() error {
	...
	// The confirm-before-fix step of the methodology is only reachable when the
	// question tool is actually on offer, so it is part of the mode's contract.
	if !got["question"] {
		return fmt.Errorf("Debug toolset missing question tool (needed to confirm the diagnosis): %+v", got)
	}
	return nil
}
