...
func codexAuthNotice(provider, managedPath string) CodexAuthNotice {
	...
	if err != nil {
		notice.Warning = true
		notice.Message = fmt.Sprintf("no ChatGPT credential found, run `foxxycode providers login %s` or sign in from Settings", provider)
		return notice
	}
	if auth.AuthMode != "" && auth.AuthMode != codexAuthModeChatGPT {
		notice.Warning = true
	...
	case strings.TrimSpace(auth.Tokens.AccessToken) == "" && !hasRefresh:
		notice.Warning = true
		notice.Message = fmt.Sprintf("credential in %s carries no tokens, run `foxxycode providers login %s`", path, provider)
	case parsed && exp.Before(time.Now()) && !hasRefresh:
		notice.Warning = true
		notice.Message = fmt.Sprintf("access token expired at %s and there is no refresh token, run `foxxycode providers login %s` (%s)",
