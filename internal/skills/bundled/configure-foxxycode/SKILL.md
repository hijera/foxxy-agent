...
- `agent` - ReAct loop model, max turns, LLM retry and pacing (`llm_retry_max` with `0` disabling retries, `llm_retry_base_ms`, `llm_min_interval_ms`, `llm_first_token_timeout_ms`), loop protection (`loop_guard`, `loop_tool_repeat_limit`, `loop_stream_repeat_cycles`, `loop_tool_cycle_repeats`, `loop_nudge_max`, `loop_stuck_action`);
