...
Switches are spelled `enable`, the same as in coddy-agent. A config written before 0.3.x spells them `enabled`: that key is still read, so address the switch as `<section>.enable` in `config_get` and `config_set` whatever the file says, and the commit writes it back under that name - never stage an `enabled` path, it is not a schema path.
