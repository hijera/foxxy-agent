...
func BuildACPConfigOptions(cfg *config.Config, state *State) []acp.ConfigOption {
	...
	modeOpt := acp.ConfigOption{
		ID:           "mode",
		Name:         "Session mode",
		Description:  "Agent executes tasks; Plan designs; Docs maintains markdown; Ask answers questions read-only; Debug diagnoses and fixes issues.",
		Category:     "mode",
		Type:         "select",
		CurrentValue: mode,
		Options: []acp.ConfigOptionValue{
			...
			{Value: string(ModeDebug), Name: "Debug", Description: "Diagnose and fix issues: hypothesize, narrow, confirm, then fix"},
		},
	}
