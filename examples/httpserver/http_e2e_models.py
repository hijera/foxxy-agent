...
1. ``GET /v1/models`` lists ``agent``, ``plan``, ``docs``, and ``ask`` with ``owned_by`` ``foxxycode`` and every YAML
...
def main() -> int:
    ...
    for need in ["agent", "plan", "docs", "ask"]:
        row = by_id.get(need)
        if row is None:
    ...
    # Switch metadata.model back to the agent default after testing the other row.
