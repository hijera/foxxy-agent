...
Requires a **running** ``foxxycode http`` (same port as ``BASE_URL``) with ``--scheduler-enabled``,
and the same ``FOXXYCODE_HOME`` and ``WORK_DIR`` the server was started with.

Steps: chat instructs run_command marker, ``foxxycode_scheduler_job_create``, then the harness waits
for cron or fallback job file, scheduler run produces ``SCHEDULER_RUN_RESULT.txt``, log lines, and
``.state`` for the job.

Environment: ``BASE_URL`` (must end with ``/v1``), ``FOXXYCODE_HOME``, ``WORK_DIR``, ``MODEL`` (optional).
"""
...
def main() -> int:
    ...
    if not base:
        ...
        return 1
    home_s = os.environ.get("FOXXYCODE_HOME", "").strip()
    work_s = os.environ.get("WORK_DIR", "").strip()
    if not home_s or not work_s:
        print("FOXXYCODE_HOME and WORK_DIR must be set to the server's home and cwd", file=sys.stderr)
        return 1
    home = Path(home_s).expanduser().resolve()
    work = Path(work_s).expanduser().resolve()
    if not home.is_dir() or not work.is_dir():
        print("FOXXYCODE_HOME and WORK_DIR must be existing directories", file=sys.stderr)
        return 1
