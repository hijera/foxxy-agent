...
| anything a plugin user can observe (the SPA, agent behaviour, `editors/**`) | a Russian `## Unreleased - <date>` entry in `editors/intellij/CHANGELOG.md` and/or `editors/vscode/CHANGELOG.md`: a merge into `main` releases at once, so the notes are part of the pull request ([.claude/rules/release-changelog.md](.claude/rules/release-changelog.md)) |
