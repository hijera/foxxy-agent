...
// legacyEnabledKey writes the switch under the name it had before the rename.
func (s *configTestFlagState) legacyEnabledKey(line int) error {
	body := "httpserver:\n  host: 127.0.0.1\n  enabled: false\n"
	if got := strings.Count(strings.TrimSuffix(body, "\n"), "\n") + 1; got != line {
		return fmt.Errorf("the fixture puts enabled on line %d, the scenario says %d", got, line)
	}
	return s.write(body)
}
