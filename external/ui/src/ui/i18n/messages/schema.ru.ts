...
export const schemaTextRu: Record<string, string> = {
  ...
  "Optional per-provider outbound proxy. Use http:// or https:// for an HTTP proxy, or socks5:// / socks5h:// for SOCKS5 (socks5h resolves hostnames via the proxy). Leave empty for a direct connection.":
    "Необязательный исходящий прокси для конкретного провайдера. Используйте http:// или https:// для HTTP-прокси либо socks5:// / socks5h:// для SOCKS5 (socks5h разрешает имена хостов через прокси). Оставьте пустым для прямого соединения.",
