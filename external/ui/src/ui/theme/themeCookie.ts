export const FOXXYCODE_UI_THEME_COOKIE = "foxxycode_ui_theme";
...
export function readUiThemeCookie(): UiThemeMode | null {
  ...
  for (const p of parts) {
    const s = p.trim();
    if (!s.startsWith(`${FOXXYCODE_UI_THEME_COOKIE}=`)) {
      continue;
    }
    const v = decodeURIComponent(
      s.slice(FOXXYCODE_UI_THEME_COOKIE.length + 1).trim(),
    );
    if (isValidTheme(v)) {
      return v;
    }
...
export function writeUiThemeCookie(mode: UiThemeMode): void {
  ...
  const secure =
    typeof window !== "undefined" && window.location.protocol === "https:"
      ? "; Secure"
      : "";
  document.cookie = `${FOXXYCODE_UI_THEME_COOKIE}=${encodeURIComponent(mode)}; Path=/; Max-Age=${MAX_AGE_SECONDS}; SameSite=Lax${secure}`;
}
