...
function SummaryHeader(props: { label: string; durationLabel: string; showChevron: boolean }) {
  return (
    <span className="thinking-left coddy-memory-copilot-head-left">
      {props.showChevron ? <span className="thinking-chevron" aria-hidden="true" /> : null}
...
export function MemoryCopilotMessage(props: {
  ...
  const legacyBusy = props.recallStatus === 'in_progress' || props.persistStatus === 'in_progress';
  const busy = memSt === 'in_progress' || (memSt == null && legacyBusy);

  const streamingBody =
    props.memoryText !== undefined
      ? props.memoryText
      : [props.recallText, props.persistText].filter((x) => x && x.length > 0).join('\n\n');
  ...
  const finished =
    memSt === 'completed' ||
    (props.recallStatus === 'completed' && props.persistStatus !== 'in_progress');
    ...
    return (
      <div className="thinking-row" data-testid="memory-copilot-row">
        <div
          ...
          aria-busy="true"
        >
          <SummaryHeader label={label} durationLabel={durationLabel} showChevron={false} />
        </div>
      </div>
    );
  }
    ...
    <div className="thinking-row">
      ...
      >
        <summary className="thinking-summary" aria-label="Memory copilot summary">
          <SummaryHeader label={label} durationLabel={durationLabel} showChevron={true} />
        </summary>
            ...
            <div className="coddy-memory-phase coddy-memory-persist">
              <div className="coddy-memory-saved-summary">
                {(props.persistTitle || '').trim() ? (
                  <span className="coddy-memory-saved-title">{(props.persistTitle || '').trim()}</span>
                ) : null}
