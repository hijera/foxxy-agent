...
func (s *Server) registerBranchRoutes() {
	s.mux.HandleFunc("POST /foxxycode/sessions/{id}/branches", s.foxxycodeBranchCreate)
	s.mux.HandleFunc("GET /foxxycode/sessions/{id}/branches", s.foxxycodeBranchList)
}

// foxxycodeBranchCreate handles POST /foxxycode/sessions/{id}/branches.
//
// Request body:
//
...
// Response:
//
//	{ "object": "foxxycode.branch_created", "newSessionId": "...", "branchIndex": N, "totalBranches": M,
//	  "fileRollbackNote": "..." }
func (s *Server) foxxycodeBranchCreate(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
		"object":           "foxxycode.branch_created",
...
// foxxycodeBranchList handles GET /foxxycode/sessions/{id}/branches.
//
// Response:
//
//	{ "object": "foxxycode.branches", "sessionId": "...", "branchPoints": [...] }
func (s *Server) foxxycodeBranchList(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimSpace(r.PathValue("id"))
	if err := session.ValidateFolderSessionID(id); err != nil {
		http.Error(w, fmt.Sprintf(`{"error":{"message":%q}}`, err.Error()), http.StatusBadRequest)
		return
