...
Use **`Makefile`** variable **`TAGS`** with **spaces** (**`make build TAGS="http ui scheduler memory miniapps cli"`**). **`go build`** uses **commas** (**`-tags=http,ui,scheduler,memory,miniapps,cli`**).
...
Official CLI binaries are published on **[GitHub Releases](https://github.com/hijera/foxxycode-agent/releases)** (assets such as **`foxxycode_0.9.3_linux_amd64.tar.gz`**). Each release matches the full feature set from **`make build TAGS="http ui scheduler memory miniapps cli"`**.
