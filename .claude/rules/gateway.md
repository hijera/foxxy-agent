...
Built with **`-tags gateway.telegram`** (Telegram only) or **`-tags gateway`** (all adapters). Without these tags `foxxycode gateway` is present in the binary but returns a "not compiled" error.
