...
- Express run (what agents run before finishing): **`make test`**
  - Runs **`make test-opencode-rules`**, **`make ui-build`**, **`make ui-test`**, then one **`go test -tags=http,ui,scheduler,memory,cli,browser,gateway,swarm ./...`** - the whole tree with every optional module compiled in, which is what the shipped binary contains. Minutes, not tens of minutes.
  - **`make ui-test`** is the SPA half: **`npm run typecheck`** (**`tsc --noEmit`** - **`vite build`** strips types with esbuild and never checks them) followed by the vitest suite. Run it after touching **`external/ui/**`**.
