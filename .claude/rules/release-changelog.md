...
## 1. Work out the version the merge will produce

CI takes the **highest SemVer tag on `origin`** and bumps the **patch**. Upstream tags
(`0.9.x`, fetched from the `upstream` remote) are irrelevant: the workflow runs
`git fetch --tags origin`, so only the fork's own tags count.

```bash
git ls-remote --tags origin | grep -oE '[0-9]+\.[0-9]+\.[0-9]+$' | sort -V | tail -1
```

Bump the last component of that. If the merge commit already carries a tag, the workflow
skips the bump and no new version is produced.

## 2. Add a changelog entry for that version
