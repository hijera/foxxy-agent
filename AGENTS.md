...
| Area | Responsibility |
|------|------------------|
| `cmd/foxxycode` | CLI entry (bare `foxxycode` console, `cli`, `acp`, `http`, `serve`, `gateway`, `sessions`, `skills`, `plugin`, `mcp`, `codex login`, `providers login`, `rules list`, `update`), plus **`-t` / `--test-config`** on the console, `acp` and `serve`: the config check in **`internal/config/check.go`** (schema stage in **`schema.go`** over the embedded **`config.schema.json`**, then the loader's rules), reported as `file:line:col` with a fix line, exit 1 on errors, nothing written; and **`--dry-run`** on the same three, which runs that check and then **`internal/dryrun`**. |
